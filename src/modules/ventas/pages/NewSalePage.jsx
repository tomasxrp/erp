...
export default function NewSalePage() {
  ...
  const addToCart = (item) => {
    ...
    if (existing) {
      ...
      if (item.itemType === 'product' && item.total_stock < 1) {
  ...
  const handleUpdateQuantity = (id, delta) => {
    setCart(prevCart => prevCart.map(item => {
      if (item.id === id) {
        ...
        // 1. Validar Stock Máximo (Solo si es Producto)
        if (delta > 0 && item.itemType === 'product' && newQuantity > item.total_stock) {
  ...
  const handleCheckout = async () => {
    ...
    if (!selectedClient && docType === 'factura') {
    ...
    try {
      ...
      // 3. Descontar Stock (SOLO PRODUCTOS)
              ...
              >
                ...
                <div className="flex justify-between items-end mt-2 z-10">
                  ...
                  {item.itemType === 'product' ? (
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${item.total_stock > 5
          ...
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
            {['boleta', 'factura'].map(type => (
              <button
                ...
                className={`flex-1 py-1.5 text-xs sm:text-sm capitalize rounded-md font-medium transition-all ${docType === type
